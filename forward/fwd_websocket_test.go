...
func TestWebSocketRequestWithOrigin(t *testing.T) {
	f, err := New(PassHostHeader(true), Logger(logrusLogger), Debug(logrusDebugFunc))
	require.NoError(t, err)
...
func TestWebSocketTransferTLSConfig(t *testing.T) {
	...
	forwarderWithoutTLSConfig, err := New(PassHostHeader(true), Logger(logrusLogger), Debug(logrusDebugFunc))
	require.NoError(t, err)
	...
	forwarderWithTLSConfig, err := New(PassHostHeader(true), RoundTripper(transport))
	require.NoError(t, err)
	...
	forwarderWithTLSConfigFromDefaultTransport, err := New(PassHostHeader(true), Logger(logrusLogger), Debug(logrusDebugFunc))
	require.NoError(t, err)
